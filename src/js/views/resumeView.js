...
import { calculateDuration, ColorLog } from "../utils";
...
export const buildResumeView = function (resumePage, pageMenu, data) {
  ...
  resumeDom.insertAdjacentHTML(
    "beforeend",
    `<div class="font_text row p-1 container"></div>`
  );
...

  {
    //! basic info
    {
      infoCol.insertAdjacentHTML(
        "afterbegin",
        Markup.image("src/img/me/ME_2.png", "responsive-img center-align")
      );
      infoCol.insertAdjacentHTML(
        "beforeend",
      ...
      //   infoCol.insertAdjacentHTML(
      //     "beforeend",
      //     '<div class="p-0 m-0 left-align"></div>'
      //   );
      //   const linksContainer = infoCol.lastChild;

      //   data.links.forEach((l) => {
      //     linksContainer.insertAdjacentHTML(
      //       "beforeend",
      //       Markup.link(l.link, l.icon, "color_icon")
      //     );
      //   });
      // }

      infoCol.insertAdjacentHTML("beforeend", Markup.p(data.info.location));
      infoCol.insertAdjacentHTML("beforeend", Markup.p(data.info.phone));
      infoCol.insertAdjacentHTML("beforeend", Markup.p(data.info.email));
    }

    //! references
    {
      infoCol.insertAdjacentHTML("beforeend", Markup.container(3, 0, ""));
      const referencesContainer = infoCol.lastChild;

      referencesContainer.insertAdjacentHTML(
        "beforeend",
        Markup.h4("References")
      );
      data.references.forEach((l) => {
        referencesContainer.insertAdjacentHTML(
          "beforeend",
          Markup.container(1, 0, "solid_l")
        );
        const personContainer = referencesContainer.lastChild;

        personContainer.insertAdjacentHTML(
          "beforeend",
          Markup.p(l.name, 0, 0, "", "color_text") +
            Markup.p(l.role) +
            Markup.p(l.phone) +
            Markup.p(l.email)
        );
