...
const iconAtlasUrl = `/src/design/icons/social.png`;
...
export const iconCSSInit = function (DOM, iconData) {
  const cSSIconTemplate = function (name, size) {
    ...
    return `.${name}{background: url(${iconAtlasUrl}) ${size}px 0px; background-size: var(--icon_background_size);}`;
  };

  // setup CSS for icon atlas
  (function () {
    DOM.iconStyleSheet.insertRule(cSSIconTemplate(iconData.icons[0].name, 0));

    for (let i = iconData.icons.length - 1; i > 0; i--) {
      DOM.iconStyleSheet.insertRule(
        cSSIconTemplate(
